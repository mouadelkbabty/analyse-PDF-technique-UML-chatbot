...
class MultiPDFComparator:
    def __init__(self, nlp_model, api_key):
        self.nlp = nlp_model
        self.api_key = api_key
        self.base_url = "https://api.mistral.ai/v1/chat/completions"
        self.documents = {}  # {filename: {"text": content, "metadata": info}}
        
    def add_document(self, filename, text, metadata):
        """Ajouter un document à la comparaison"""
    ...
    def search_information(self, query):
        """Rechercher une information dans tous les documents"""
        ...
        for filename, doc_data in self.documents.items():
            ...
            # Vérifier si la requête est présente dans le document
            contains_info = query_lower in text_lower
            
            # Extraire des passages pertinents (contexte autour de la requête)
            relevant_passages = []
            if contains_info:
                import re
                # Trouver toutes les occurrences et extraire le contexte
                for match in re.finditer(re.escape(query_lower), text_lower):
                    start = max(0, match.start() - 200)
                    end = min(len(text_lower), match.end() + 200)
                    context = doc_data["text"][start:end].strip()
                    relevant_passages.append(context)
            
            results[filename] = {
                "contains_info": contains_info,
                "passages": relevant_passages[:3],  # Limiter à 3 passages
                "score": len(relevant_passages)
            }
        
        return results
    
    def ask_comparative_question(self, question):
        """Poser une question comparative sur tous les documents"""
        if not self.documents:
            return "❌ Aucun document chargé."
        
        # Préparer le contexte combiné
        combined_context = ""
        for filename, doc_data in self.documents.items():
            # Limiter le contexte de chaque document
            doc_text = doc_data["text"][:3000]  # Limiter à 3000 caractères par doc
            combined_context += f"\n\n--- DOCUMENT: {filename} ---\n{doc_text}"
        
        system_prompt = f"""Tu es un assistant IA spécialisé dans la comparaison de documents. Tu as accès au contenu de plusieurs documents PDF et tu dois répondre aux questions en français en te basant uniquement sur ce contenu.
...
- Compare et analyse les informations présentes dans les différents documents
- Indique clairement dans quel document tu trouves chaque information
- Si une information n'est présente que dans certains documents, précise-le
- Sois précis et factuel
- Structure ta réponse en mentionnant les noms des documents"""
...
class TechnicalDocumentExtractor:
    ...
    def extract_metadata_from_pdf(self, pdf_file):
        """Extraction des métadonnées du PDF avec dates formatées (day/month/year)"""
        metadata = {}
        
        def format_pdf_date(raw_date):
            """Convertit une date PDF en format jj/mm/aaaa"""
            if raw_date and raw_date.startswith("D:"):
                try:
                    dt = datetime.strptime(raw_date[2:10], "%Y%m%d")  # On prend juste AAAAMMJJ
                    return dt.strftime("%d/%m/%Y")
                except Exception:
                    return raw_date  # Si le format est inhabituel, retourne brut
            return raw_date
        ...
        try:
            with pdfplumber.open(pdf_file) as pdf:
                if pdf.metadata:
                    ...
                    metadata['pdf_creation_date'] = format_pdf_date(pdf.metadata.get('CreationDate', ''))
                    metadata['pdf_modification_date'] = format_pdf_date(pdf.metadata.get('ModDate', ''))
...
def main():
    ...
    main_tabs = st.tabs(["📄 Upload & Extraction", "💬 Chat avec le Document", "📊 Comparaison Multi-PDF"])
    ...
    with main_tabs[2]:
        # Nouvel onglet Comparaison Multi-PDF
        st.subheader("📊 Comparaison Multi-PDF")
        st.markdown("**Téléchargez vos fichiers PDF et comparez-les**")
        
        # Initialiser le comparateur s'il n'existe pas
        if 'pdf_comparator' not in st.session_state:
            if mistral_api_key:
                st.session_state.pdf_comparator = MultiPDFComparator(nlp, mistral_api_key)
            else:
                st.session_state.pdf_comparator = None
        
        if not mistral_api_key:
            st.warning("⚠️ Veuillez configurer votre clé API Mistral AI dans la sidebar pour utiliser cette fonctionnalité")
            return
        ...
        for i in range(3):
            with st.container():
                ...
                if uploaded_file:
                    uploaded_files.append((i, uploaded_file))
        
        # Traitement des fichiers uploadés
        if uploaded_files:
            # Effacer les anciens documents
            if st.session_state.pdf_comparator:
                st.session_state.pdf_comparator.clear_documents()
            
            # Organiser l'affichage en colonnes selon le nombre de fichiers
            ...
            for idx, (file_idx, uploaded_file) in enumerate(uploaded_files):
                with cols[idx]:
                    st.markdown(f"### 📄 {uploaded_file.name}")
                    
                    # Extraire le texte
                    with st.spinner(f"Extraction du texte de {uploaded_file.name}..."):
                        try:
                            text, metadata = extract_text_from_pdf(uploaded_file)
                            document_data[uploaded_file.name] = {
                                "text": text,
                                "metadata": metadata
                            }
                            ...
                            if st.session_state.pdf_comparator:
                                st.session_state.pdf_comparator.add_document(
                                    uploaded_file.name, text, metadata
                                )
                            
                            st.success(f"✅ Traité ({len(text)} caractères)")
                            st.info(f"📄 {metadata['pages']} pages")
                            ...
                            with st.expander(f"👀 Aperçu de {uploaded_file.name}"):
                                ...
                                st.text_area(
                                    ...
                                    preview_text, 
                                    height=150,
                                    key=f"preview_{file_idx}"
                                )
                        
                        except Exception as e:
                            st.error(f"❌ Erreur lors du traitement de {uploaded_file.name}: {e}")
            
            # Section de recherche et comparaison
            if document_data:
                st.markdown("---")
                st.markdown("### 🔍 Recherche et Comparaison")
                
                # Tabs pour différents types de recherche
                search_tabs = st.tabs(["🎯 Recherche Ciblée", "💬 Question Comparative"])
                
                with search_tabs[0]:
                    st.markdown("**Recherchez une information spécifique dans les documents**")
                    
                    search_query = st.text_input(
                        "Terme à rechercher :",
                        placeholder="Ex: sécurité, architecture, base de données...",
                        key="search_query"
                    )
                    
                    if st.button("🔍 Rechercher", key="search_btn") and search_query:
                        if st.session_state.pdf_comparator:
                            with st.spinner("Recherche en cours..."):
                                search_results = st.session_state.pdf_comparator.search_information(search_query)
                            
                            st.markdown("#### 📋 Résultats de recherche")
                            
                            # Organiser l'affichage des résultats
                            result_cols = st.columns(len(document_data))
                            
                            for idx, (filename, result) in enumerate(search_results.items()):
                                with result_cols[idx]:
                                    # Couleur selon la présence de l'information
                                    if result["contains_info"]:
                                        # Vert si contient l'information
                                        st.markdown(f"""
                                        <div style='background-color:#d4edda; border: 2px solid #c3e6cb; padding:15px; border-radius:10px; margin-bottom:10px;'>
                                            <h4 style='color:#155724; margin:0;'>✅ {filename}</h4>
                                            <p style='color:#155724; margin:5px 0;'><strong>Trouvé ({result["score"]} occurrences)</strong></p>
                                        </div>
                                        """, unsafe_allow_html=True)
                                        
                                        # Afficher les passages pertinents
                                        for i, passage in enumerate(result["passages"]):
                                            with st.expander(f"Extrait {i+1}"):
                                                # Mettre en surbrillance le terme recherché
                                                highlighted_passage = passage.replace(
                                                    search_query, 
                                                    f"**{search_query}**"
                                                )
                                                st.markdown(highlighted_passage)
                                    else:
                                        # Rouge si ne contient pas l'information
                                        st.markdown(f"""
                                        <div style='background-color:#f8d7da; border: 2px solid #f5c6cb; padding:15px; border-radius:10px; margin-bottom:10px;'>
                                            <h4 style='color:#721c24; margin:0;'>❌ {filename}</h4>
                                            <p style='color:#721c24; margin:5px 0;'><strong>Non trouvé</strong></p>
                                        </div>
                                        """, unsafe_allow_html=True)
                
                with search_tabs[1]:
                    st.markdown("**Posez une question comparative sur tous les documents**")
                    
                    comparative_question = st.text_input(
                        "Question comparative :",
                        placeholder="Ex: Quelles sont les différences de sécurité entre ces documents ?",
                        key="comparative_question"
                    )
                    
                    # Questions suggérées pour la comparaison
                    with st.expander("💡 Questions suggérées"):
                        suggested_comparative = [
                            "Quelles sont les principales différences entre ces documents ?",
                            "Quels documents mentionnent la sécurité et comment ?",
                            "Comparez les technologies utilisées dans chaque document",
                            "Quels sont les points communs entre ces documents ?",
                            "Quel document est le plus technique ?",
                            "Quelles dates importantes sont mentionnées dans chaque document ?"
                        ]
                        
                        for suggestion in suggested_comparative:
                            if st.button(f"💬 {suggestion}", key=f"comp_suggest_{hash(suggestion)}"):
                                comparative_question = suggestion
                    
                    if st.button("🚀 Analyser", key="compare_btn") and comparative_question:
                        if st.session_state.pdf_comparator:
                            with st.spinner("Analyse comparative en cours..."):
                                comparative_response = st.session_state.pdf_comparator.ask_comparative_question(comparative_question)
                            
                            st.markdown("#### 🤖 Analyse Comparative")
                            st.markdown(comparative_response)
                
                # Section de statistiques
                st.markdown("---")
                st.markdown("### 📊 Statistiques des Documents")
                
                stats_cols = st.columns(len(document_data))
                
                for idx, (filename, data) in enumerate(document_data.items()):
                    with stats_cols[idx]:
                        st.markdown(f"**{filename}**")
                        st.metric("Caractères", f"{len(data['text']):,}")
                        st.metric("Pages", data['metadata']['pages'])
                        st.metric("Mots (approx.)", f"{len(data['text'].split()):,}")
        
        else:
            st.info("📁 Téléchargez au moins un fichier PDF pour commencer la comparaison")
